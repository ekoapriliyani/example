    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Detail Inspeksi Incoming Project') }}
            </h2>
            <div class="flex gap-2">
                ...
                <a href="{{ route('incomingproject.inspeksi', $incomingproject->id) }}"
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200">
                <div class="p-6">
                    <div class="flex items-center gap-2 mb-4">
                        ...
                        <h3 class="text-lg font-bold text-gray-800">Hasil Inspeksi Incoming Project</h3>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 text-sm text-left">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 font-semibold text-gray-900">Material</th>
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @forelse ($incomingproject->incomingprojectinspeksi as $inp)
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        <td class="px-4 py-3">{{ $loop->iteration }}</td>
                                        <td class="px-4 py-3">{{ $inp->user->name ?? 'N/A' }}</td>
                                        <td class="px-4 py-3">{{ $inp->material->description ?? 'N/A' }}</td>
                                        <td class="px-4 py-3">
                                            @if ($inp->visual === 'OK')
                                                <span
                                        ...
                                        <td class="px-4 py-3">
                                            <button type="button" class="text-sm text-indigo-600 hover:underline"
                                                onclick="toggleImage({{ $inp->id }})">
                                                Lihat Gambar
                                            </button>
                                        </td>

                                        <td class="px-4 py-3 text-center bg-blue-50/30">{{ $inp->created_at }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-4 py-8 text-center text-gray-400 italic">Belum
                                            ada
                                            data Inspeksi Incoming Project.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                ...
                {{-- modal gambar inp --}}
                @foreach ($incomingproject->incomingprojectinspeksi as $inp)
                    <div id="image-{{ $inp->id }}"
                        ...
                        <div class="bg-white rounded-lg shadow-lg w-3/4 p-6 max-h-[80vh] overflow-y-auto">
                            ...
                            <div class="mt-4 text-right">
                                <button onclick="toggleImage({{ $inp->id }})"
