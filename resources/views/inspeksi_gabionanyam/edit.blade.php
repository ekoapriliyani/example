                ...
                <div class="p-8 text-gray-900">
                    <div class="mb-6">
                        <p class="text-sm text-gray-600">
                            Silakan ubah detail operasional inspeksi QC Gabionanyam.
                        </p>
                    </div>

                    <form action="{{ route('inspeksi_gabionanyam.update', $inspeksiGabionanyam->id) }}" method="POST"
                        class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <x-text-input id="nomor_inspeksi" name="nomor_inspeksi" type="text"
                                ...
                                value="{{ old('nomor_inspeksi', $inspeksiGabionanyam->nomor_inspeksi) }}" readonly />
                        ...
                        <div>
                            ...
                            <x-text-input id="tanggal" name="tanggal" type="date" class="mt-1 block w-full"
                                value="{{ old('tanggal', $inspeksiGabionanyam->tanggal) }}" required />
                        ...
                        <div>
                            ...
                            <select id="pro_id" name="pro_id"
                                ...
                                @foreach ($pros as $pro)
                                    <option value="{{ $pro->id }}"
                                        {{ old('pro_id', $inspeksiGabionanyam->pro_id) == $pro->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <x-text-input id="pro_description" type="text" class="mt-1 block w-full bg-gray-100"
                                value="{{ old('pro_description', $inspeksiGabionanyam->pro ? $inspeksiGabionanyam->pro->description : '') }}"
                                readonly />
                        </div>
                        ...
                        <div>
                            ...
                            <x-text-input id="pro_qty" type="text" class="mt-1 block w-full bg-gray-100"
                                value="{{ old('pro_qty', $inspeksiGabionanyam->pro ? $inspeksiGabionanyam->pro->qty : '') }}"
                                readonly />
                        </div>
                        ...
                        <div>
                            ...
                            <select id="shift" name="shift"
                                ...
                                <option value="1"
                                    {{ old('shift', $inspeksiGabionanyam->shift) == '1' ? 'selected' : '' }}>
                                    Shift 1
                                </option>
                                <option value="2"
                                    {{ old('shift', $inspeksiGabionanyam->shift) == '2' ? 'selected' : '' }}>
                                    Shift 2
                                </option>
                                <option value="3"
                                    {{ old('shift', $inspeksiGabionanyam->shift) == '3' ? 'selected' : '' }}>
                                    Shift 3
                                </option>
                            </select>
                            ...
                            <select id="mesin_id" name="mesin_id"
                                ...
                                @foreach ($mesins as $mesin)
                                    <option value="{{ $mesin->id }}"
                                        {{ old('mesin_id', $inspeksiGabionanyam->mesin_id) == $mesin->id ? 'selected' : '' }}>
                            ...
                            <div class="flex items-center space-x-2 mt-1">
                                <div class="relative flex-1">
                                    <x-text-input id="total_prod" name="total_prod" type="number" step="0.01"
                                        class="block w-full"
                                        value="{{ old('total_prod', $inspeksiGabionanyam->total_prod) }}"
                                        placeholder="0.00" />
                                </div>
                                <div class="w-32">
                                    <select id="satuan" name="satuan"
                                        ...
                                        <option value="unit"
                                            {{ old('satuan', $inspeksiGabionanyam->satuan) == 'unit' ? 'selected' : '' }}>
                                            unit
                                        </option>
                                        <option value="pcs"
                                            {{ old('satuan', $inspeksiGabionanyam->satuan) == 'pcs' ? 'selected' : '' }}>
                                            pcs
                                        </option>
                                        <option value="kg"
                                            {{ old('satuan', $inspeksiGabionanyam->satuan) == 'kg' ? 'selected' : '' }}>
                                            kg
                                        </option>
                                        <option value="roll"
                                            {{ old('satuan', $inspeksiGabionanyam->satuan) == 'roll' ? 'selected' : '' }}>
                                            roll
                                        </option>
                                        <option value="lembar"
                                            {{ old('satuan', $inspeksiGabionanyam->satuan) == 'lembar' ? 'selected' : '' }}>
                                            lembar
                                        </option>
                                        <option value="ton"
                                            {{ old('satuan', $inspeksiGabionanyam->satuan) == 'ton' ? 'selected' : '' }}>
                                            ton
                                        </option>
                                    </select>
                                </div>
