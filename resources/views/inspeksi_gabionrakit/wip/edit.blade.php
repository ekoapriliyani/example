<x-app-layout>
        ...
        <div class="flex items-center justify-between">
            ...
            <p class="text-sm text-gray-500">
                ...
                <span class="font-mono font-bold text-indigo-600">
                    {{ $inspeksi_gabionrakit->nomor_inspeksi }}
                </span>
            </p>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="mx-auto max-w-4xl sm:px-6 lg:px-8">
            <div class="overflow-hidden border border-gray-200 bg-white shadow-sm sm:rounded-lg">
                <div class="p-8">
                    <form action="{{ route('inspeksi_gabionrakit_wip.update', $wip->id) }}" method="POST"
                        enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')

                        <input type="hidden" name="inspeksi_gabionrakit_id"
                            value="{{ old('inspeksi_gabionrakit_id', $wip->inspeksi_gabionrakit_id) }}">
                        ...
                        <div class="grid grid-cols-2 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                <x-input-label for="jml_sekat" :value="__('Jumlah Sekat')" />
                                <x-text-input id="jml_sekat" name="jml_sekat" type="number" step="0.01"
                                    class="mt-1 block w-full" :value="old('jml_sekat', $wip->jml_sekat)" />
                                <x-input-error class="mt-2" :messages="$errors->get('jml_sekat')" />
                            </div>
                            <div>
                                ...
                                <select id="type" name="type"
                                    ...
                                    <option value="Galvanized"
                                        {{ old('type', $wip->type) == 'Galvanized' ? 'selected' : '' }}>
                                        Galvanized
                                    </option>
                                    ...
                                    <div class="detail-row grid grid-cols-1 gap-4 md:grid-cols-3">
                                        <div>
                                            ...
                                            <select name="detail_description[]"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                                @include('inspeksi_gabionrakit.wip.options-detail', [
                        ...
                        <div class="flex items-center justify-end gap-4 border-t border-gray-100 pt-6">
                            <a href="{{ route('inspeksi_gabionrakit.show', $inspeksi_gabionrakit->id) }}"
    ...
    <script>
        const optionsDetail = `@include('inspeksi_gabionrakit.wip.options-detail', ['selected' => null])`;
