    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
                ...
                @php
                    $isApproved = $inspeksi_pvc->approval_status === 'APPROVED';
                @endphp
                <div class="flex items-center gap-2">
                    ...
                    @else
                        <a href="{{ route('inspeksi_pvc.wip', $inspeksi_pvc->id) }}"
        ...
        <div class="mx-auto max-w-7xl space-y-8 sm:px-6 lg:px-8">
            ...
            <div class="overflow-hidden border border-gray-200 bg-white shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <div class="mb-4 flex items-center gap-2">
                        ...
                        <h3 class="text-lg font-bold text-gray-800">Hasil Inspeksi WIP PVC/HDPE</h3>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 text-left text-sm">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 font-semibold text-gray-900">C3</th>
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @forelse ($inspeksi_pvc->inspeksiPvcWip as $wip)
                                    <tr class="transition-colors hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex items-center gap-3">
                                                ...
                                                @else
                                                    <a href="{{ route('inspeksi_pvc_wip.edit', $wip->id) }}"
                                                ...
                                                @else
                                                    <form action="{{ route('inspeksi_pvc_wip.destroy', $wip->id) }}"
                                        ...
                                        <td class="bg-blue-50/30 px-4 py-3 text-center">{{ $wip->c3 }}</td>
                                        <td class="bg-blue-50/30 px-4 py-3 text-center">{{ $wip->c4 }}</td>
                                        <td class="bg-blue-50/30 px-4 py-3 text-center">{{ $wip->ch }}</td>
                                        <td class="bg-blue-50/30 px-4 py-3 text-center">{{ $wip->d_kawat_inti }}</td>
                                        <td class="bg-blue-50/30 px-4 py-3 text-center">{{ $wip->d_kawat_pvc }}</td>
            ...
            <div class="overflow-hidden border border-gray-200 bg-white shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <div class="overflow-x-auto rounded-lg border border-gray-200">
                        {{-- modal detail wip --}}
                        @foreach ($inspeksi_pvc->inspeksiPvcWip as $wip)
                    ...
                    <div class="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            @if (in_array(auth()->user()->role, ['supervisor', 'manager', 'administrator']))
                                <form id="approval-form-{{ $inspeksi_pvc->id }}"
                                    action="{{ route('inspeksi-pvc.toggle', $inspeksi_pvc->id) }}" method="POST"
                                    class="hidden">
                                    @csrf
                                    @method('PATCH')
                                </form>
                                <button type="button"
                                    onclick="confirmApproval({{ $inspeksi_pvc->id }}, '{{ $inspeksi_pvc->isApproved() ? 'unapprove' : 'approve' }}')"
                                    class="{{ $inspeksi_pvc->isApproved()
    ...
    <div id="print-section" class="hidden">
        ...
        <table width="100%" cellpadding="5" cellspacing="0"
            ...
            <tr>
                ...
                <td style="width: 30%;">: {{ $inspeksi_pvc->nomor_inspeksi }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">PRO Number</td>
                <td>: {{ $inspeksi_pvc->pro->pro_id }}</td>
                <td style="font-weight: bold;">Description</td>
                <td>: {{ $inspeksi_pvc->pro->description }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">QTY Ordered</td>
                <td>: {{ $inspeksi_pvc->pro->qty }}</td>
                <td style="font-weight: bold;">Total Prod Per Shift</td>
                <td>: {{ $inspeksi_pvc->total_prod }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Shift</td>
                <td>: {{ $inspeksi_pvc->shift }}</td>
                <td style="font-weight: bold;">Mesin</td>
                <td>: {{ $inspeksi_pvc->mesin->nama_mesin }}</td>
            </tr>
        </table>
            ...
            <tr>
                <td style="width: 50%; vertical-align: top;">
                    ...
                    <p style="margin: 0; border-top: 1px solid #000; width: 200px; padding-top: 5px;">
                        {{ $inspeksi_pvc->inspeksiPvcWip->first()->user->name ?? '.................' }}</p>
