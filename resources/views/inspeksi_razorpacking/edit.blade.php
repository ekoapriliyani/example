        ...
        <div class="mx-auto max-w-3xl sm:px-6 lg:px-8">
            <div class="overflow-hidden border border-gray-200 bg-white shadow-sm sm:rounded-lg">
                <div class="p-8 text-gray-900">
                    ...
                    <form action="{{ route('inspeksi_razorpacking.update', $inspeksi_razorpacking->id) }}" method="POST"
                        class="space-y-6">
                        @csrf
                        @method('PUT')
                        <div>
                            ...
                            <x-text-input id="nomor_inspeksi" name="nomor_inspeksi" type="text"
                                ...
                                value="{{ old('nomor_inspeksi', $inspeksi_razorpacking->nomor_inspeksi) }}" readonly />
                        </div>
                        <div>
                            <x-input-label for="tanggal" :value="__('Tanggal Inspeksi')" />
                            <x-text-input id="tanggal" name="tanggal" type="date"
                                class="mt-1 block w-full bg-gray-100"
                                value="{{ old('tanggal', $inspeksi_razorpacking->tanggal) }}" />
                        </div>
                            ...
                            <select id="pro_id" name="pro_id"
                                ...
                                @foreach ($pros as $pro)
                                    <option value="{{ $pro->id }}" data-description="{{ $pro->description }}"
                                        {{ old('pro_id', $inspeksi_razorpacking->pro_id) == $pro->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <x-text-input id="description" type="text" class="mt-1 block w-full bg-gray-100"
                                :value="old('description', optional($inspeksi_razorpacking->pro)->description)" readonly />
                        </div>
                        ...
                        <div>
                            ...
                            <select id="shift" name="shift"
                                ...
                                <option value="1"
                                    {{ old('shift', $inspeksi_razorpacking->shift) == '1' ? 'selected' : '' }}>Shift 1
                                </option>
                                <option value="2"
                                    {{ old('shift', $inspeksi_razorpacking->shift) == '2' ? 'selected' : '' }}>Shift 2
                                </option>
                                <option value="3"
                                    {{ old('shift', $inspeksi_razorpacking->shift) == '3' ? 'selected' : '' }}>Shift 3
                                </option>
                            </select>
