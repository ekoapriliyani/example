<x-app-layout>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200">
                <div class="p-8 text-gray-900">
                    ...
                    <form action="{{ route('inspeksi_shearing.update', $inspeksiShearing->id) }}" method="POST"
                        class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="nomor_inspeksi" name="nomor_inspeksi" type="text"
                                class="mt-1 block w-full bg-gray-100" :value="old('nomor_inspeksi', $inspeksiShearing->nomor_inspeksi)" readonly />
                        </div>

                        <div>
                            <x-input-label for="tanggal" :value="__('Tanggal')" />
                            <x-text-input id="tanggal" name="tanggal" type="date" class="mt-1 block w-full"
                                :value="old('tanggal', $inspeksiShearing->tanggal)" required />
                        ...
                        <div>
                            ...
                            <select id="pro_id" name="pro_id"
                                ...
                                @foreach ($pros as $pro)
                                    <option value="{{ $pro->id }}"
                                        {{ old('pro_id', $inspeksiShearing->pro_id) == $pro->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <x-text-input id="pro_description" type="text" class="mt-1 block w-full bg-gray-100"
                                :value="old('pro_description', $inspeksiShearing->pro->description)" readonly />
                        </div>
                        ...
                        <div>
                            ...
                            <x-text-input id="pro_qty" type="text" class="mt-1 block w-full bg-gray-100"
                                :value="old('pro_qty', $inspeksiShearing->pro->qty)" readonly />
                        </div>
                        ...
                        <div>
                            ...
                            <select id="shift" name="shift"
                                ...
                                <option value="1"
                                    {{ old('shift', $inspeksiShearing->shift) == '1' ? 'selected' : '' }}>
                                    Shift 1
                                </option>

                                <option value="2"
                                    {{ old('shift', $inspeksiShearing->shift) == '2' ? 'selected' : '' }}>
                                    Shift 2
                                </option>

                                <option value="3"
                                    {{ old('shift', $inspeksiShearing->shift) == '3' ? 'selected' : '' }}>
                                    Shift 3
                                </option>
                            </select>
                            ...
                            <select id="mesin_id" name="mesin_id"
                                ...
                                @foreach ($mesins as $mesin)
                                    <option value="{{ $mesin->id }}"
                                        {{ old('mesin_id', $inspeksiShearing->mesin_id) == $mesin->id ? 'selected' : '' }}>
                            ...
                            <div class="flex items-center space-x-2 mt-1">
                                <div class="relative flex-1">
                                    <x-text-input id="total_prod" name="total_prod" type="number" step="0.01"
                                        class="block w-full"
                                        value="{{ old('total_prod', $inspeksiShearing->total_prod) }}"
                                        placeholder="0.00" />
                                </div>
                                <div class="w-32">
                                    <select id="satuan" name="satuan"
                                        ...
                                        <option value="unit"
                                            {{ old('satuan', $inspeksiShearing->satuan) == 'unit' ? 'selected' : '' }}>
                                            unit
                                        </option>
                                        <option value="pcs"
                                            {{ old('satuan', $inspeksiShearing->satuan) == 'pcs' ? 'selected' : '' }}>
                                            pcs
                                        </option>
                                        <option value="roll"
                                            {{ old('satuan', $inspeksiShearing->satuan) == 'roll' ? 'selected' : '' }}>
                                            roll
                                        </option>
                                        <option value="lembar"
                                            {{ old('satuan', $inspeksiShearing->satuan) == 'lembar' ? 'selected' : '' }}>
                                            lembar
                                        </option>
                                    </select>
                                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            loadProDetail("{{ old('pro_id', $inspeksiShearing->pro_id) }}");
