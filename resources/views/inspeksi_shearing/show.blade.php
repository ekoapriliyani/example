    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold leading-tight text-gray-800">
                {{ __('Detail Inspeksi Shearing') }}
            </h2>
            <div class="flex gap-2">
                <a href="{{ route('inspeksi_shearing.index') }}"
                ...
                @php
                    $isApproved = $inspeksi_shearing->approval_status === 'APPROVED';
                @endphp
                <div class="flex items-center gap-2">
                    ...
                    @else
                        <a href="{{ route('inspeksi_shearing.wip', $inspeksi_shearing->id) }}"
            ...
            <div class="overflow-hidden border border-gray-200 bg-white shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <div class="mb-4 flex items-center gap-2">
                        ...
                        <h3 class="text-lg font-bold text-gray-800">Hasil Inspeksi WIP Shearing</h3>
                    </div>
                    ...
                    <div class="overflow-x-auto rounded-lg border border-gray-200">
                        <table class="min-w-full divide-y divide-gray-200 text-left text-sm">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @forelse ($inspeksi_shearing->inspeksiShearingWip as $wip)
                                    <tr class="transition-colors hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex items-center gap-3">
                                                ...
                                                @else
                                                    <a href="{{ route('inspeksi_shearing_wip.edit', $wip->id) }}"
                                                ...
                                                @else
                                                    <form
                                                        action="{{ route('inspeksi_shearing_wip.destroy', $wip->id) }}"
                ...
                <div class="p-6">
                    <div class="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                        ...
                        <div class="flex items-center gap-4">
                            <!-- Status -->
                            @if ($inspeksi_shearing->isApproved())
                                <span
                            ...
                            @if (in_array(auth()->user()->role, ['supervisor', 'manager', 'administrator']))
                                <form id="approval-form-{{ $inspeksi_shearing->id }}"
                                    action="{{ route('inspeksi-shearing.toggle', $inspeksi_shearing->id) }}"
                                    method="POST" class="hidden">
                                    @csrf
                                    @method('PATCH')
                                </form>
                                <button type="button"
                                    onclick="confirmApproval({{ $inspeksi_shearing->id }}, '{{ $inspeksi_shearing->isApproved() ? 'unapprove' : 'approve' }}')"
                                    class="{{ $inspeksi_shearing->isApproved()
    ...
    <div id="print-section" class="hidden">
        ...
        <table width="100%" cellpadding="5" cellspacing="0"
            ...
            <tr>
                ...
                <td style="width: 30%;">: {{ $inspeksi_shearing->nomor_inspeksi }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">PRO Number</td>
                <td>: {{ $inspeksi_shearing->pro->pro_id }}</td>
                <td style="font-weight: bold;">Description</td>
                <td>: {{ $inspeksi_shearing->pro->description }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">QTY Ordered</td>
                <td>: {{ $inspeksi_shearing->pro->qty }}</td>
                <td style="font-weight: bold;">Total Prod Per Shift</td>
                <td>: {{ $inspeksi_shearing->total_prod }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Shift</td>
                <td>: {{ $inspeksi_shearing->shift }}</td>
                <td style="font-weight: bold;">Mesin</td>
                <td>: {{ $inspeksi_shearing->mesin->nama_mesin }}</td>
            </tr>
        </table>
            ...
            <table width="100%" cellpadding="4" cellspacing="0"
                ...
                <thead>
                    <tr style="background-color: #f0f0f0;">
                        ...
                        <th style="border: 1px solid #000; padding: 5px; text-align: center; width: 7%;">L Potong</th>
            ...
            <tr>
                <td style="width: 50%; vertical-align: top;">
                    ...
                    <p style="margin: 0; border-top: 1px solid #000; width: 200px; padding-top: 5px;">
                        {{ $inspeksi_shearing->inspeksiShearingWip->first()->user->name ?? '.................' }}</p>
                ...
                <td style="width: 50%; vertical-align: top;">
                    ...
                    <br><br><br>
                    <p style="margin: 0; border-top: 1px solid #000; width: 200px; padding-top: 5px;">
                        {{ $inspeksi_shearing->approver->name ?? '.................' }}</p>
