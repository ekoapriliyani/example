<x-app-layout>
        ...
        <div class="flex items-center justify-between">
            ...
            <p class="text-sm text-gray-500">
                ...
                <span class="font-mono font-bold text-indigo-600">
                    {{ $inspeksi_shearing->nomor_inspeksi }}
                </span>
            </p>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="mx-auto max-w-4xl sm:px-6 lg:px-8">
            <div class="overflow-hidden border border-gray-200 bg-white shadow-sm sm:rounded-lg">
                <div class="p-8">
                    <form action="{{ route('inspeksi_shearing_wip.update', $wip->id) }}" method="POST"
                        enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')

                        <input type="hidden" name="inspeksi_shearing_id"
                            value="{{ old('inspeksi_shearing_id', $wip->inspeksi_shearing_id) }}">
                        ...
                        <div class="grid grid-cols-2 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <x-text-input id="p_potong" name="p_potong" type="number" step="0.01"
                                    class="mt-1 block w-full" :value="old('p_potong', $wip->p_potong)" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="l_potong" name="l_potong" type="number" step="0.01"
                                    class="mt-1 block w-full" :value="old('l_potong', $wip->l_potong)" required />
                                    ...
                                    <div class="detail-row grid grid-cols-1 gap-4 md:grid-cols-3">
                                        <div>
                                            ...
                                            <select name="detail_description[]"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                                @include('inspeksi_shearing.wip.options-detail', [
    ...
    <script>
        const optionsDetail = `@include('inspeksi_shearing.wip.options-detail', ['selected' => null])`;
