<x-app-layout>
        ...
        <div class="flex items-center justify-between">
            ...
            <p class="text-sm text-gray-500">
                ...
                <span class="font-mono font-bold text-indigo-600">
                    {{ $inspeksi_slitting->nomor_inspeksi }}
                </span>
            </p>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="mx-auto max-w-4xl sm:px-6 lg:px-8">
            <div class="overflow-hidden border border-gray-200 bg-white shadow-sm sm:rounded-lg">
                <div class="p-8">
                    <form action="{{ route('inspeksi_slitting_wip.update', $wip->id) }}" method="POST"
                        enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')

                        <input type="hidden" name="inspeksi_slitting_id"
                            value="{{ old('inspeksi_slitting_id', $wip->inspeksi_slitting_id) }}">
                        ...
                        <div class="grid grid-cols-2 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <x-text-input id="l_sheetgalvanized" name="l_sheetgalvanized" type="number"
                                    step="0.01" class="mt-1 block w-full" :value="old('l_sheetgalvanized', $wip->l_sheetgalvanized)" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="tebal_sheetgalvanized" name="tebal_sheetgalvanized" type="number"
                                    step="0.01" class="mt-1 block w-full" :value="old('tebal_sheetgalvanized', $wip->tebal_sheetgalvanized)" required />
                                    ...
                                    <div class="detail-row grid grid-cols-1 gap-4 md:grid-cols-3">
                                        <div>
                                            ...
                                            <select name="detail_description[]"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                                @include('inspeksi_slitting.wip.options-detail', [
                        ...
                        <div class="flex items-center justify-end gap-4 border-t border-gray-100 pt-6">
                            <a href="{{ route('inspeksi_slitting.show', $inspeksi_slitting->id) }}"
    ...
    <script>
        const optionsDetail = `@include('inspeksi_slitting.wip.options-detail', ['selected' => null])`;
