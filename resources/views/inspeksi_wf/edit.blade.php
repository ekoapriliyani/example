<x-app-layout>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200">
                <div class="p-8 text-gray-900">
                    ...
                    <form action="{{ route('inspeksi_wf.update', $inspeksi_wf->id) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <x-text-input id="nomor_inspeksi" name="nomor_inspeksi" type="text"
                                ...
                                value="{{ old('nomor_inspeksi', $inspeksi_wf->nomor_inspeksi) }}" readonly />
                        </div>
                        <div>
                            <x-input-label for="tanggal" :value="__('Tanggal Inspeksi')" />
                            <x-text-input id="tanggal" name="tanggal" type="date"
                                class="mt-1 block w-full bg-gray-100"
                                value="{{ old('tanggal', $inspeksi_wf->tanggal) }}" />
                        </div>
                            ...
                            <select id="pro_id" name="pro_id"
                                ...
                                @foreach ($pros as $pro)
                                    <option value="{{ $pro->id }}" data-description="{{ $pro->description }}"
                                        {{ old('pro_id', $inspeksi_wf->pro_id) == $pro->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <x-text-input id="description" type="text" class="mt-1 block w-full bg-gray-100"
                                :value="old('description', optional($inspeksi_wf->pro)->description)" readonly />
                        </div>
                        ...
                        <div>
                            ...
                            <select id="shift" name="shift"
                                ...
                                <option value="1" {{ old('shift', $inspeksi_wf->shift) == '1' ? 'selected' : '' }}>
                                    Shift 1
                                </option>
                                <option value="2"
                                    {{ old('shift', $inspeksi_wf->shift) == '2' ? 'selected' : '' }}>Shift 2
                                </option>
                                <option value="3"
                                    {{ old('shift', $inspeksi_wf->shift) == '3' ? 'selected' : '' }}>Shift 3
                                </option>
                            </select>
                            ...
                            <select id="mesin_id" name="mesin_id"
                                ...
                                @foreach ($mesins as $mesin)
                                    <option value="{{ $mesin->id }}"
                                        {{ old('mesin_id', $inspeksi_wf->mesin_id) == $mesin->id ? 'selected' : '' }}>
                            ...
                            <div class="flex items-center space-x-2 mt-1">
                                <div class="relative flex-1">
                                    <x-text-input id="total_prod" name="total_prod" type="number" step="0.01"
                                        class="block w-full" value="{{ old('total_prod', $inspeksi_wf->total_prod) }}"
                                        placeholder="0.00" />
                                </div>
                                <div class="w-32">
                                    <select id="satuan" name="satuan"
                                        ...
                                        <option value="unit"
                                            {{ old('satuan', $inspeksi_wf->satuan) == 'unit' ? 'selected' : '' }}>unit
                                        </option>
                                        <option value="pcs"
                                            {{ old('satuan', $inspeksi_wf->satuan) == 'pcs' ? 'selected' : '' }}>pcs
                                        </option>
                                        <option value="kg"
                                            {{ old('satuan', $inspeksi_wf->satuan) == 'kg' ? 'selected' : '' }}>kg
                                        </option>
                                        <option value="roll"
                                            {{ old('satuan', $inspeksi_wf->satuan) == 'roll' ? 'selected' : '' }}>roll
                                        </option>
                                        <option value="lembar"
                                            {{ old('satuan', $inspeksi_wf->satuan) == 'lembar' ? 'selected' : '' }}>
                                            lembar
                                        </option>
                                        <option value="ton"
                                            {{ old('satuan', $inspeksi_wf->satuan) == 'ton' ? 'selected' : '' }}>ton
                                        </option>
                                    </select>
                                </div>
                            </div>
    ...
    {{--  --}}
    <script>
        $('#product_wf_ref_id').select2({
            placeholder: '-- Pilih Product wf --',
            allowClear: true,
            width: '100%'
        });
    </script>
