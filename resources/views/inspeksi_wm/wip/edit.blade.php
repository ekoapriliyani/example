    ...
    <div class="py-12">
        <div class="mx-auto max-w-4xl sm:px-6 lg:px-8">
            <div class="overflow-hidden border border-gray-200 bg-white shadow-sm sm:rounded-lg">
                <div class="p-8">
                    <form action="{{ route('inspeksi_wm_wip.update', $wip->id) }}" method="POST"
                        enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')

                        <input type="hidden" name="inspeksi_wip_id"
                            value="{{ old('inspeksi_wm_id', $wip->inspeksi_wm_id) }}">
                        ...
                        <div class="grid grid-cols-2 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                <x-input-label for="selisih_diagonal" :value="__('Selisih Diagonal')" />
                                <x-text-input id="selisih_diagonal" name="selisih_diagonal" type="number"
                                    step="0.01" class="mt-1 block w-full" :value="old('selisih_diagonal', $wip->selisih_diagonal)" required />
                            ...
                            <div>
                                <x-input-label for="l_product_act" :value="__('L Product Act')" />
                            ...
                            <div>
                                <x-input-label for="p_mesh_act" :value="__('P Mesh Act')" />
                                <x-text-input id="p_mesh_act" name="p_mesh_act" type="number" step="0.01"
                                    class="mt-1 block w-full" :value="old('p_mesh_act', $wip->p_mesh_act)" required />
                                <x-input-error class="mt-2" :messages="$errors->get('p_mesh_act')" />
                            </div>
                            <div>
                                <x-input-label for="l_mesh_act" :value="__('L Mesh Act')" />
                                <x-text-input id="l_mesh_act" name="l_mesh_act" type="number" step="0.01"
                                    class="mt-1 block w-full" :value="old('l_mesh_act', $wip->l_mesh_act)" required />
                            ...
                            <div>
                                ...
                                <select id="torsi_strength" name="torsi_strength"
                                    ...
                                    required>
                                    <option value="">-- Pilih Torsi Strength --</option>
                                    <option value="OK"
                                        {{ old('torsi_strength', $wip->torsi_strength) == 'OK' ? 'selected' : '' }}>
                                        OK
                                    </option>
                                    <option value="NG"
                                        {{ old('torsi_strength', $wip->torsi_strength) == 'NG' ? 'selected' : '' }}>
                                        NG
                                    </option>
                                </select>
                            ...
                            <div>
                                <x-input-label for="status_dimensi" :value="__('Status Dimensi')" />
                                <select id="status_dimensi" name="status_dimensi"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                    required>
                                    <option value="">-- Pilih Status Dimensi --</option>
                                    <option value="OK"
                                        {{ old('status_dimensi', $wip->status_dimensi) == 'OK' ? 'selected' : '' }}>
                                        OK
                                    </option>
                                    <option value="NG"
                                        {{ old('status_dimensi', $wip->status_dimensi) == 'NG' ? 'selected' : '' }}>
                                        NG
                                    </option>
                                </select>
                                    ...
                                    <div class="detail-row grid grid-cols-1 gap-4 md:grid-cols-3">
                                        <div>
                                            ...
                                            <select name="detail_description[]"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                                @include('inspeksi_wm.fg.options-detail', [
