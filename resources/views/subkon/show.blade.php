<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('subkon.edit', $subkon->id) }}"
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200">
                <div class="p-6 sm:p-8 text-gray-900">
                    <dl class="-my-3 divide-y divide-gray-100 text-sm">
                        ...
                        <div class="grid grid-cols-1 gap-1 py-4 sm:grid-cols-3 sm:gap-4">
                            <dt class="font-semibold text-gray-900">Nama Subkon</dt>
                            <dd class="text-gray-700 text-base sm:col-span-2">{{ $subkon->name }}</dd>
                        </div>

                        <div class="grid grid-cols-1 gap-1 py-4 sm:grid-cols-3 sm:gap-4">
                            <dt class="font-semibold text-gray-900">Terdaftar Pada</dt>
                            <dd class="text-gray-600 sm:col-span-2 italic">
                                {{ $subkon->created_at->format('d M Y, H:i') }}
                            </dd>
                        </div>
                    </dl>
                </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-red-200">
                <div class="p-4 bg-red-50 flex items-center justify-between">
                    <form id="delete-form-{{ $subkon->id }}" action="{{ route('subkon.destroy', $subkon->id) }}"
                        method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="button" onclick="confirmDelete({{ $subkon->id }}, '{{ $subkon->subkon_id }}')"
    ...
    <script>
        function confirmDelete(id, name) {
            Swal.fire({
                title: 'Hapus Subkon?',
                text: "Subkon " + name + " akan dihapus secara permanen!",
                icon: 'warning',
